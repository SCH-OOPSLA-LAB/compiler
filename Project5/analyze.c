...
static void checkNode(TreeNode* syntaxTree)
{
	...
	case DecK:
		...
		case ArrVarK:
			syntaxTree->expressionType = IntegerArray;
			break;
	...
	case ExpK:
		...
		case OpK:
			...
			if ((syntaxTree->op == PLUS) || (syntaxTree->op == MINUS) ||
				(syntaxTree->op == TIMES) || (syntaxTree->op == OVER))
			{
				if ((syntaxTree->child[0]->expressionType == Integer) &&
			...
			/* Relational operators */
				...
				(syntaxTree->op == LE) || (syntaxTree->op == GE) ||
		...
		case IdK:
			...
			else if (syntaxTree->declaration->expressionType == IntegerArray)
			{
				if (syntaxTree->child[0] == NULL)
					syntaxTree->expressionType = IntegerArray;
				else
...
void markGlobals(TreeNode* syntaxTree)
{
	...
	while (cursor != NULL)
	{
		if ((cursor->nodekind == DecK) &&
			((cursor->kind.dec == VarK) ||
				(cursor->kind.dec == ArrVarK)))
		{
